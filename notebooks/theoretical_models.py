...
class recall:
    def model(*, k: int, n: int, m: int, b: int, k_b: int) -> float:
        return float((k_b + scipy.stats.binom.cdf(k_b-1, np.arange(k_b, k), p=1/b).sum()) / k)

    def simulation(*, k: int, n: int, m: int, b: int, k_b: int, reps: int) -> tuple[float, float]:
        samples = np.array([
            (np.random.choice(n, size=n, replace=False) < k).reshape(b, -1).sum(-1).clip(max=k_b).sum() / k
            for _ in range(reps)
        ])
        return float(samples.mean()), float(samples.std())

    def simulation_birthday(*, k: int, n: int, m: int, b: int, k_b: int, reps: int) -> tuple[float, float]:
        samples = np.array([
            np.bincount(np.random.randint(0, b, k), minlength=b).clip(max=k_b).sum() / k
            for _ in range(reps)
        ])
        return float(samples.mean()), float(samples.std())
...
class cost:
    ...
    def approx_topk(cls, *, k: int, n: int, m: int, b: int, k_b: int) -> int:
        return cls.topk(k=k_b, n=n//b, m=m*b) + (k_b*b > k) * cls.topk(k=k, n=k_b*b, m=m)
...
if __name__ == "__main__":
    ...
    with Path("theoretical_models.jsonl").open("w") as f, multiprocessing.Pool() as pool:
        for results in tqdm.tqdm(pool.imap(_run_test_knm, settings, chunksize=1), total=len(settings)):
