...
__device__ std::tuple<IndexType, IndexType, IndexType> calculateBucket(
  ...
  if (interleaved) {
    ...
    auto previousNormalBuckets =
        bucketId > remainder ? bucketId - remainder : 0;
    inputStartIndex = previousBigBuckets * (baseInputBucketSize + 1) +
        previousNormalBuckets * baseInputBucketSize;
    inputIndexStride = 1;
  }
...
__device__ void correctIndices(
    ...
    IndexType indexCorrectionOffset,
    IndexType indexCorrectionStride) {
#pragma unroll
  for (IndexType i = 0; i < J; i++) {
    largestIndices[i] =
...
__global__ void blockTopk(
  ...
  BlockMergeSort(temp_storage_shuffle)
      .Sort(largestValues, largestIndices, Descending());
...
void launchKernel(
  ...
  if (should_use_multithread_buckets) {
    ...
    blockTopk<T, IndexType, Dim, J, kNumThreads>
...
void priority_queue_topk(
  ...
  TORCH_CHECK(
      numDims <= MAX_DIMS, "topk: input tensor has too many dimensions");
